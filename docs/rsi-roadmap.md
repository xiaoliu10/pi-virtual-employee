...
## 第 4 层：配置/提示词 A/B（0.2.66 已落地）

`prompt.rules` 是**热数据**：改完不用打包发布，下个会话就生效——所以它是迭代最快的一层，也是最容易悄悄改坏的一层。0.2.66 落地的分工是：**候选由模型写，评分由代码算，生效由管理员定**。

- **评测用例**（`eval_cases`，首次启动自动播种红线用例）：`prompt_includes`/`prompt_excludes` 断言装配后的系统提示词里有没有某段文字（零模型调用，可以放开跑）；`reply_matches`/`reply_excludes` 用 input 跑一个**隔离回合**再断言回复（要模型调用，测的是真行为）。
- **评分与一票否决**（`prompt_lab action=run`）：候选必须**严格优于基线**才被推荐；任一**关键用例**失败即否决，无论平均分多高。关键用例的停用本身也按护栏变更处理——需要「确认」，且此后每次评分都会明说「这次没覆盖哪些关键用例」。
- **隔离**：评测跑在 `eval:` 前缀的会话里，用**会话级 prompt 覆盖**而不是写 config——真实会话永远拿不到候选，进程崩了也不会留下半生效的实验；评测回合不落库、不进遥测（否则改进回路会测量自己的实验，污染它要改进的信号）。
- **回滚**：`prompt_history` 记录每次变更的前后文本、理由、分数，`action=rollback` 只能回滚最近一次（回滚更早的会静默丢掉后续改动，那不是任何人的本意）。
- **单一装配路径**：生产会话与评测共用 `promptPartsFor()`。评测器若自己拼一份"差不多的提示词"，它测的就是另一篇文章，分数全是假的。
- 护栏化：用例集与一票否决判定都在不可变清单里（`prompt-lab-cases`/`prompt-lab-veto`），改它们必须显式 ACK。

**下一步（未做）**：`prompt_extra`/技能描述也纳入同一套评测；用 judge 模型给开放类用例打分；把「哪个变体在生产里表现更好」用遥测闭环（灰度到部分会话而不是全局生效）。
...
- **爆炸半径**：那个 profile 里是明文 LLM API key、钉钉 appSecret、Gitee/OSS 令牌、带登录态的浏览器 profile，机器还能访问内网业务系统。任何「自主改代码 + 自主发布」的设计都必须假设这四条会被滥用——这就是第 2 层不可跳过的原因。
