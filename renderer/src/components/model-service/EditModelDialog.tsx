			...
			<div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
				...
				<div className="space-y-4">
					...
					<label className="block">
						<span className="mb-2 block text-sm font-medium text-slate-800">
							上下文窗口 <span className="ml-1 cursor-help text-slate-300" title="真实上下文窗口（tokens）。中转/别名模型务必按网关实际值填写，否则长对话会被错误压缩甚至空回复；留空 = 继承默认。">?</span>
						</span>
					...
					<label className="block">
						<span className="mb-2 block text-sm font-medium text-slate-800">
							最大输出 Token <span className="ml-1 cursor-help text-slate-300" title="单次回复的最大输出长度（tokens）。中转模型继承到不合适的基础值时会截断长回复；留空 = 继承默认。">?</span>
						</span>
				...
				<div className="mt-6 flex items-center justify-between">
					...
					<div className="flex gap-3">
						...
						<button type="button" onClick={save} className="rounded-xl bg-blue-500 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-600">保存</button>
					</div>
				</div>
			</div>
		</div>
