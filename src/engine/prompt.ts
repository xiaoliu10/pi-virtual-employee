...
function capabilityRules(c: RulesCtx): string {
	...
	lines.push(
			...
			"- **权限改动的目标必须由对方明确给出，不能自行推断**：权限管理只在单聊进行（群里无法可靠鉴别操作者），而单聊里没有「当前群」这个语境——对方说「给群里的人设权限」「给这个群开权限」时你并不知道是哪个群，**先问清或先列出候选让对方确认**（manage_access list 列出最近 IM 会话，list_members 列出某会话里出现过的人），复述「我理解是「XX 群」（<会话 ID>），成员是 A、B、C」并等确认后再执行；绝不能挑一个看起来像的群直接改，也不要把 staffId 猜着填。" +
			"机器人只登记**给机器人发过消息的人**，名单天然不全（平台未必授予列群成员权限），要如实说明，不要凭群名推测成员。给整个群开 admin 通常是过度授权——admin 是系统级权限（完整命令/系统设置/权限管理），若对方只是想让这个群能用某项能力，先提示按人设 operator 或对该群设门槛（set_conversation）这两个更小的做法。" +
